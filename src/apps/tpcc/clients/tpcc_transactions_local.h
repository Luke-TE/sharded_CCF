...
namespace tpcc
{
  ...
  private:
    ...
    District find_district(int32_t w_id, int32_t d_id)
    {
      ...
      auto districts = client_read_writer.get_district(key);
      ...
      int32_t w_id, int32_t d_id, int32_t o_id, int32_t number)
    {
      ...
      return client_read_writer.get_order_line(key);
    }

    std::optional<Item> find_item(int32_t id)
    {
      Item::Key key = {id};
      return client_read_writer.get_item(key);
    }

    Stock find_stock(int32_t w_id, int32_t s_id)
    {
      Stock::Key key = {s_id, w_id};
      auto stock = client_read_writer.get_stock(key);
      if (!stock.has_value())
      {
    ...
    Customer find_customer(int32_t w_id, int32_t d_id, int32_t c_id)
    {
      ...
      auto customers = client_read_writer.get_customer(table_key, key);
    ...
    Warehouse find_warehouse(int32_t w_id)
    {
      ...
      auto warehouse = client_read_writer.get_warehouse(key);
    ...
    void insert_order(Order& o)
    {
      ...
      client_read_writer.put_order(table_key, o.get_key(), o);
    }
    ...
    {
      client_read_writer.put_order_line(line.get_key(), line);
    }
      ...
      int32_t w_id, int32_t d_id, const char* c_last)
    {
      ...
      std::string last_name(c_last);
      return client_read_writer.get_customer_by_name(table_key, last_name).value();
    }
      ...
      const int32_t w_id, const int32_t d_id, const int32_t c_id)
    {
      ...
      return client_read_writer.get_last_order_by_customer(table_key, key).value();
    }
    ...
    Order find_order(int32_t w_id, int32_t d_id, int32_t o_id)
    {
      ...
      Order::Key key = {o_id};
      return client_read_writer.get_order(table_key, key).value();
    }
      ...
      std::vector<DeliveryOrderInfo>* orders)
    {
      for (int32_t d_id = 1; d_id <= District::NUM_PER_WAREHOUSE; ++d_id)
      {
        ...
        table_key.v.d_id = d_id;
        auto new_order = client_read_writer.get_last_new_order(table_key);

        if (!new_order.has_value()) {
          continue;
        }
        int32_t o_id = new_order.value().o_id;
        client_read_writer.remove_new_order(new_order.value().get_key());
      ...
      PaymentOutput* output)
    {
      ...
      client_read_writer.put_history(h.get_key(), h);
    }
  ...
  public:
    ClientReadWriter client_read_writer;

    TpccTransactionsClient(ClientReadWriter c, uint32_t seed) :
      client_read_writer(c)
    {
